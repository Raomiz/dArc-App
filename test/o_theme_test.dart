...
void main() {
  test('Dennis-locked vault tokens are exact hex', () {
    expect(OColors.ground, const Color(0xFF0C0712));
    expect(OColors.surface, const Color(0xFF160B1E));
    ...
    expect(OColors.night, OColors.ground);
    expect(OColors.field, OColors.surface);
    expect(OColors.byzantine, OColors.purpose);
    expect(OColors.jade, OColors.intention);
    expect(OColors.gold, OColors.commit);

    expect(OColors.ground, isNot(const Color(0xFF050505)));
    expect(OColors.ground, isNot(const Color(0xFF121212)));
  });

  test('theme seats vault ground and token roles', () {
    final theme = buildOTheme();
    expect(theme.scaffoldBackgroundColor, OColors.ground);
  ...
  test('whisper labels are tracked; Commit type is high contrast', () {
    ...
    expect(OType.commit.color, OColors.ground);
  ...
  testWidgets('Commit button is gold threshold, not a bland submit', (tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        theme: buildOTheme(),
        home: Scaffold(
          backgroundColor: OColors.ground,
          body: CommitButton(
